<?php
namespace MascotCoreElectricien\CPT\Footer;
 ...
 * Singleton class
 * class CPT_Footer
 * @package MascotCoreElectricien\CPT\Footer;
 */
final class CPT_Footer implements Lib\Mascot_Core_Electricien_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Footer', 'mascot-core-electricien' );
		$this->ptPluralName = esc_html__( 'Parts - Footer', 'mascot-core-electricien' );
		$this->ptKey = 'footer';
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-footer'	=> esc_html__( 'Currently Active Footer', 'mascot-core-electricien' ),
		...
		switch( $columns ) {
			case 'active-footer' :
				if( mascot_core_electricien_theme_installed() ) {
					$active_footer_id = electricien_get_redux_option( 'footer-settings-choose-footer-widget-area', 'default' );
