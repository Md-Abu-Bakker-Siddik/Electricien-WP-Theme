<?php
namespace MascotCoreElectricien\CPT\SidePushPanel;
 ...
 * @package MascotCoreElectricien\CPT\SidePushPanel;
 */
final class CPT_SidePushPanel implements Lib\Mascot_Core_Electricien_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-electricien' );
		$this->ptPluralName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-electricien' );
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-side-push-panel'	=> esc_html__( 'Currently Active Side Push Panel', 'mascot-core-electricien' ),
		...
		switch( $columns ) {
			case 'active-side-push-panel' :
				if( mascot_core_electricien_theme_installed() ) {
					$active_headertop_id = electricien_get_redux_option( 'header-settings-choose-side-push-panel-cpt-widget-area', 'default' );
