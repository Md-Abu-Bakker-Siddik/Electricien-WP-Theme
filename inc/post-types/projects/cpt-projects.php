<?php
namespace MascotCoreElectricien\CPT\Projects;
 ...
 * Singleton class
 * class CPT_Projects
 * @package MascotCoreElectricien\CPT\Projects;
 */
final class CPT_Projects implements Lib\Mascot_Core_Electricien_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Projects Item', 'mascot-core-electricien' );
		$this->ptPluralName = esc_html__( 'Projects', 'mascot-core-electricien' );
		$this->ptKey = 'projects';
	...
	public function register() {
		if ( class_exists( 'ReduxFramework' ) ) {
			if( ! mascot_core_electricien_get_redux_option( 'cpt-settings-projects-enable', true ) ) {
				return;
			}
		}

		$this->ptPluralName = mascot_core_electricien_get_redux_option( 'cpt-settings-projects-label', esc_html__( 'Projects', 'mascot-core-electricien' ) );
		$this->ptMenuIcon = mascot_core_electricien_get_redux_option( 'cpt-settings-projects-admin-dashicon', $this->ptMenuIcon );
		$this->ptKeyRewriteBase = mascot_core_electricien_get_redux_option( 'cpt-settings-projects-slug', $this->ptKeyRewriteBase );
		$this->ptTaxKeyRewriteBase = mascot_core_electricien_get_redux_option( 'cpt-settings-projects-cat-slug', $this->ptKeyRewriteBase );
	...
	private function registerCustomTax() {
		...
		$labels = array(
			'name'						=> _x( 'Projects Categories', 'Taxonomy General Name', 'mascot-core-electricien' ),
			'singular_name'				=> _x( 'Project Category', 'Taxonomy Singular Name', 'mascot-core-electricien' ),
			'menu_name'					=> $this->ptSingularName . esc_html__( ' Categories', 'mascot-core-electricien' ),
	...
	public function regMetaBoxes( $meta_boxes ) {
		$meta_boxes[] = array(
			...
			'title'	  => esc_html__( 'Projects Meta Box', 'mascot-core-electricien' ),
			'post_types' => $this->ptKey,
			'priority'   => 'high',
			'fields'	 => array(
				array(
					'id'     => 'electricien_' . 'projects_mb_settings',
					// Group field
					'type'   => 'group',
					// Clone whole group?
					'clone'  => false,
					...
					'fields' => array(
						array(
							'id'	=> 'project_subtitle',
							'name'		=> esc_html__( 'Project Sub Title', 'mascot-core-electricien' ),
							'type'	=> 'text',
						),
					),
				),
